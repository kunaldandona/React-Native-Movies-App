...
class People extends Component {
    ...
    fetchMovieData = () => {
        const Url = `https://api.themoviedb.org/3/search/person?api_key=8367b1854dccedcfc9001204de735470&language=en-US&query=${this.state.text}&page=1`
        fetch(Url)
            .then(data => data.json())
                .then(data => {
                    this.setState({
                    ...
                    {this.state.results.slice(0, 10).map((result, i) => {
                        return (
                            <Card key={i} style={styles.movieInfo}>
                                <View>
                                <Thumbnail square large style={styles.movieImage} source={{uri: `https://image.tmdb.org/t/p/original/${result.profile_path}`}} /> 
                                </View>
